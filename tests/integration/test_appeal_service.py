...
@pytest.mark.asyncio
async def test_appeal_priority_boost_disabled_by_global_redemption_toggle(monkeypatch, integration_engine) -> None:
    from app.config import settings
    ...
    async with session_factory() as session:
        async with session.begin():
            appellant = User(tg_user_id=88622, username="appeal_global_disabled")
            session.add(appellant)
            await session.flush()
            ...
            appeal = await create_appeal_from_ref(
                session,
                appellant_user_id=appellant.id,
                appeal_ref="manual_global_redemption_disabled",
            )
            ...
            session.add(
                PointsLedgerEntry(
                    ...
                    amount=20,
                    event_type=PointsEventType.FEEDBACK_APPROVED,
                    dedupe_key="seed:appeal:boost:global_disabled",
                    reason="seed",
                    payload=None,
                )
            )
...
@pytest.mark.asyncio
async def test_appeal_priority_boost_requires_min_account_age(monkeypatch, integration_engine) -> None:
    from app.config import settings
    ...
    async with session_factory() as session:
        async with session.begin():
            appellant = User(tg_user_id=88623, username="appeal_min_account_age")
            session.add(appellant)
            await session.flush()
            ...
            appeal = await create_appeal_from_ref(
                session,
                appellant_user_id=appellant.id,
                appeal_ref="manual_min_account_age",
            )
            ...
            session.add(
                PointsLedgerEntry(
                    ...
                    amount=20,
                    event_type=PointsEventType.FEEDBACK_APPROVED,
                    dedupe_key="seed:appeal:boost:min_account_age",
                    reason="seed",
                    payload=None,
                )
            )
...
@pytest.mark.asyncio
async def test_appeal_priority_boost_respects_weekly_spend_cap(monkeypatch, integration_engine) -> None:
    from app.config import settings
    ...
    async with session_factory() as session:
        async with session.begin():
            appellant = User(tg_user_id=88625, username="appeal_weekly_cap")
            session.add(appellant)
            await session.flush()
            ...
            appeal = await create_appeal_from_ref(
                session,
                appellant_user_id=appellant.id,
                appeal_ref="manual_weekly_spend_cap",
            )
            ...
            session.add(
                PointsLedgerEntry(
                    ...
                    amount=100,
                    event_type=PointsEventType.FEEDBACK_APPROVED,
                    dedupe_key="seed:appeal:boost:weekly_cap:balance",
                    reason="seed",
                    payload=None,
                )
            )
            session.add(
                PointsLedgerEntry(
                    ...
                    dedupe_key="seed:appeal:boost:weekly_cap:spent",
                    reason="seed",
                    payload=None,
                )
            )
...
@pytest.mark.asyncio
async def test_appeal_priority_boost_respects_monthly_spend_cap(monkeypatch, integration_engine) -> None:
    from app.config import settings
    ...
    async with session_factory() as session:
        async with session.begin():
            appellant = User(tg_user_id=88627, username="appeal_monthly_cap")
            session.add(appellant)
            await session.flush()
            ...
            appeal = await create_appeal_from_ref(
                session,
                appellant_user_id=appellant.id,
                appeal_ref="manual_monthly_spend_cap",
            )
            ...
            session.add(
                PointsLedgerEntry(
                    ...
                    amount=100,
                    event_type=PointsEventType.FEEDBACK_APPROVED,
                    dedupe_key="seed:appeal:boost:monthly_cap:balance",
                    reason="seed",
                    payload=None,
                )
            )
            session.add(
                PointsLedgerEntry(
                    ...
                    amount=-10,
                    event_type=PointsEventType.APPEAL_PRIORITY_BOOST,
                    dedupe_key="seed:appeal:boost:monthly_cap:spent",
                    reason="seed",
                    payload=None,
                )
            )
...
@pytest.mark.asyncio
async def test_appeal_priority_boost_respects_global_daily_spend_cap(monkeypatch, integration_engine) -> None:
    from app.config import settings
    ...
    async with session_factory() as session:
        async with session.begin():
            appellant = User(tg_user_id=88651, username="appeal_global_daily_spend_cap")
            session.add(appellant)
            await session.flush()
            ...
            appeal = await create_appeal_from_ref(
                session,
                appellant_user_id=appellant.id,
                appeal_ref="manual_global_daily_spend_cap",
            )
            ...
            session.add(
                PointsLedgerEntry(
                    ...
                    amount=100,
                    event_type=PointsEventType.FEEDBACK_APPROVED,
                    dedupe_key="seed:appeal:global:spend:balance",
                    reason="seed",
                    payload=None,
                )
            )
            session.add(
                PointsLedgerEntry(
                    ...
                    amount=-10,
                    event_type=PointsEventType.FEEDBACK_PRIORITY_BOOST,
                    dedupe_key="seed:appeal:global:spend:used",
                    reason="seed",
                    payload=None,
                    created_at=datetime.now(UTC),
                )
