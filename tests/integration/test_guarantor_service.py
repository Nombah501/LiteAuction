...
@pytest.mark.asyncio
async def test_guarantor_priority_boost_respects_weekly_spend_cap(monkeypatch, integration_engine) -> None:
    from app.config import settings
    ...
    async with session_factory() as session:
        async with session.begin():
            submitter = User(tg_user_id=93345, username="guarant_weekly_cap")
            session.add(submitter)
            await session.flush()
            ...
            created = await create_guarantor_request(
                session,
                submitter_user_id=submitter.id,
                details="Проверка недельного лимита списаний на бусты гаранта",
            )
            assert created.item is not None
            ...
            session.add(
                PointsLedgerEntry(
                    ...
                    amount=100,
                    event_type=PointsEventType.FEEDBACK_APPROVED,
                    dedupe_key="seed:guarant:boost:weekly_cap:balance",
                    reason="seed",
                    payload=None,
                )
            )
            session.add(
                PointsLedgerEntry(
                    ...
                    dedupe_key="seed:guarant:boost:weekly_cap:spent",
                    reason="seed",
                    payload=None,
                )
            )
...
@pytest.mark.asyncio
async def test_guarantor_priority_boost_respects_weekly_redemption_limit(monkeypatch, integration_engine) -> None:
    from app.config import settings
    ...
    async with session_factory() as session:
        async with session.begin():
            submitter = User(tg_user_id=93346, username="guarant_weekly_limit")
            session.add(submitter)
            await session.flush()
            ...
            created = await create_guarantor_request(
                session,
                submitter_user_id=submitter.id,
                details="Проверка недельного лимита числа бустов гаранта",
            )
            assert created.item is not None
            ...
            session.add(
                PointsLedgerEntry(
                    ...
                    amount=100,
                    event_type=PointsEventType.FEEDBACK_APPROVED,
                    dedupe_key="seed:guarant:boost:weekly_limit:balance",
                    reason="seed",
                    payload=None,
                )
            )
            session.add(
                PointsLedgerEntry(
                    ...
                    amount=-10,
                    event_type=PointsEventType.GUARANTOR_PRIORITY_BOOST,
                    dedupe_key="seed:guarant:boost:weekly_limit:used",
                    reason="seed",
                    payload=None,
                )
            )
