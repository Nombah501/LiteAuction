...
@pytest.mark.asyncio
async def test_feedback_priority_boost_requires_min_earned_points(monkeypatch, integration_engine) -> None:
    from app.config import settings
    ...
    async with session_factory() as session:
        async with session.begin():
            submitter = User(tg_user_id=93044, username="boost_min_earned_submitter")
            session.add(submitter)
            await session.flush()
            ...
            created = await create_feedback(
                ...
                submitter_user_id=submitter.id,
                feedback_type=FeedbackType.BUG,
                content="Проверка минимально заработанных points для буста",
            )
            assert created.item is not None
            ...
            session.add(
                PointsLedgerEntry(
                    ...
                    amount=20,
                    event_type=PointsEventType.FEEDBACK_APPROVED,
                    dedupe_key="seed:feedback:boost:min_earned_points",
                    reason="seed",
                    payload=None,
                )
            )
...
@pytest.mark.asyncio
async def test_feedback_priority_boost_respects_weekly_redemption_limit(monkeypatch, integration_engine) -> None:
    from app.config import settings
    ...
    async with session_factory() as session:
        async with session.begin():
            submitter = User(tg_user_id=93046, username="boost_weekly_limit_submitter")
            session.add(submitter)
            await session.flush()
            ...
            created = await create_feedback(
                ...
                submitter_user_id=submitter.id,
                feedback_type=FeedbackType.BUG,
                content="Проверка недельного лимита числа бустов",
            )
            assert created.item is not None
            ...
            session.add(
                PointsLedgerEntry(
                    ...
                    amount=100,
                    event_type=PointsEventType.FEEDBACK_APPROVED,
                    dedupe_key="seed:feedback:boost:weekly_limit:balance",
                    reason="seed",
                    payload=None,
                )
            )
            session.add(
                PointsLedgerEntry(
                    ...
                    amount=-10,
                    event_type=PointsEventType.FEEDBACK_PRIORITY_BOOST,
                    dedupe_key="seed:feedback:boost:weekly_limit:used",
                    reason="seed",
                    payload=None,
                )
            )
...
@pytest.mark.asyncio
async def test_feedback_priority_boost_respects_monthly_spend_cap(monkeypatch, integration_engine) -> None:
    from app.config import settings
    ...
    async with session_factory() as session:
        async with session.begin():
            submitter = User(tg_user_id=93047, username="boost_monthly_cap_submitter")
            session.add(submitter)
            await session.flush()
            ...
            created = await create_feedback(
                ...
                submitter_user_id=submitter.id,
                feedback_type=FeedbackType.BUG,
                content="Проверка месячного лимита списаний на бусты",
            )
            assert created.item is not None
            ...
            session.add(
                PointsLedgerEntry(
                    ...
                    amount=100,
                    event_type=PointsEventType.FEEDBACK_APPROVED,
                    dedupe_key="seed:feedback:boost:monthly_cap:balance",
                    reason="seed",
                    payload=None,
                )
            )
            session.add(
                PointsLedgerEntry(
                    ...
                    amount=-10,
                    event_type=PointsEventType.FEEDBACK_PRIORITY_BOOST,
                    dedupe_key="seed:feedback:boost:monthly_cap:spent",
                    reason="seed",
                    payload=None,
                )
            )
